/*
 * FarmTech Solutions - Sistema Final Otimizado ESP32
 * 
 * Gerenciamento de Sensores e Display LCD com Otimizações de Memória
 * 
 * OTIMIZAÇÕES IMPLEMENTADAS:
 * - Tipos de dados menores (uint8_t, uint16_t, int8_t)
 * - Strings no flash (F() macro)
 * - Estruturas compactas
 * - JSON otimizado
 * - Controle PID eficiente
...
// =============================================================================
// CONFIGURAÇÕES OTIMIZADAS - TIPOS MENORES PARA ECONOMIA DE MEMÓRIA
// =============================================================================

// OTIMIZAÇÃO: uint8_t para pinos (economiza 2 bytes por variável)
...
const uint16_t INTERVALO_WIFI_CHECK = 30000;  // 30 segundos
...
const uint8_t UMIDADE_AR_MIN = 40;
const uint8_t UMIDADE_AR_MAX = 80;

// OTIMIZAÇÃO: uint8_t para configurações booleanas
const uint8_t ENABLE_SERIAL_PLOTTER = 1;
const uint8_t ENABLE_CSV_FORMAT = 1;
const uint8_t ENABLE_WIFI = 1;

// =============================================================================
// ESTRUTURA DE DADOS OTIMIZADA - 50% ECONOMIA DE MEMÓRIA
// =============================================================================
...
// =============================================================================
// VARIÁVEIS GLOBAIS OTIMIZADAS - TIPOS MENORES
// =============================================================================

// OTIMIZAÇÃO: Objetos de hardware
LiquidCrystal_I2C lcd(LCD_ADDRESS, LCD_COLS, LCD_ROWS);
DHT dht(DHT_PIN, DHT_TYPE);

// OTIMIZAÇÃO: Controle de tempo usando uint32_t
uint32_t ultima_leitura = 0;
uint32_t ultima_atualizacao_display = 0;
uint32_t ultima_serial_plotter = 0;
uint32_t ultima_verificacao_wifi = 0;
...
// OTIMIZAÇÃO: Contadores e flags usando uint8_t
uint8_t contador_leituras = 0;
uint8_t contador_erros = 0;
uint8_t modo_display = 0;       // 0=normal, 1=detalhado, 2=config
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println(F("FarmTech Solutions - Sistema Final Otimizado"));
  Serial.println(F("Gerenciamento de Sensores e Display LCD"));
  
  // OTIMIZAÇÃO: Cabeçalho Serial Plotter usando F()
  ...
  // OTIMIZAÇÃO: Inicialização do Display LCD
  inicializarDisplay();
  
  // OTIMIZAÇÃO: Inicialização dos sensores
  inicializarSensores();
  
  // OTIMIZAÇÃO: Conectar WiFi se habilitado
  if (ENABLE_WIFI) {
    conectarWiFi();
  }
  
  // OTIMIZAÇÃO: Primeira leitura dos sensores
  lerSensores();
  
  Serial.println(F("Sistema final otimizado inicializado com sucesso!"));
}
...
void loop() {
  ...
  if (ENABLE_WIFI && tempo_atual - ultima_verificacao_wifi >= INTERVALO_WIFI_CHECK) {
    verificarConexaoWiFi();
    ultima_verificacao_wifi = tempo_atual;
  }
...
// =============================================================================
// FUNÇÃO OTIMIZADA DE INICIALIZAÇÃO DO DISPLAY
// =============================================================================
...
void inicializarDisplay() {
  ...
  lcd.setCursor(0, 1);
  lcd.print(F("Sistema Final"));
  lcd.setCursor(0, 2);
  lcd.print(F("Otimizado"));
  lcd.setCursor(0, 3);
  lcd.print(F("Inicializando..."));
  
  delay(3000);
  
  // OTIMIZAÇÃO: Limpar e mostrar tela principal
  lcd.clear();
  lcd.setCursor(0, 0);
...
// =============================================================================
// FUNÇÃO OTIMIZADA DE INICIALIZAÇÃO DOS SENSORES
// =============================================================================

void inicializarSensores() {
  // OTIMIZAÇÃO: Configuração de pinos
  pinMode(SOIL_MOISTURE_PIN, INPUT);
  pinMode(RELAY_PIN, OUTPUT);
  pinMode(LED_STATUS_PIN, OUTPUT);
  
  // OTIMIZAÇÃO: Estado inicial
  digitalWrite(RELAY_PIN, LOW);
  digitalWrite(LED_STATUS_PIN, LOW);
  
  // OTIMIZAÇÃO: Inicialização DHT22
  dht.begin();
  
  Serial.println(F("Sensores inicializados com sucesso"));
}

// =============================================================================
// FUNÇÃO OTIMIZADA DE GERENCIAMENTO DE SENSORES
// =============================================================================

void gerenciarSensores() {
  // OTIMIZAÇÃO: Leitura dos sensores
  lerSensores();
  
  // OTIMIZAÇÃO: Análise e controle
  analisarDados();
  controlarIrrigacaoPID();
  
  // OTIMIZAÇÃO: Envio de dados se WiFi disponível
  if (ENABLE_WIFI && wifi_conectado) {
    enviarDadosServidor();
  }
  
  // OTIMIZAÇÃO: Atualizar contadores
  contador_leituras++;
  
  // OTIMIZAÇÃO: Log otimizado
  Serial.println(F("=== CICLO DE LEITURA COMPLETO ==="));
  Serial.printf(F("Leitura #%d - Status: %d\n"), contador_leituras, status_sistema);
}
...
void lerSensores() {
  ...
  // OTIMIZAÇÃO: Atualizar estrutura de dados otimizada
...
void analisarDados() {
  ...
  } else if (umidade_ar < UMIDADE_AR_MIN * 100) {
    status_sistema = 5; // ALERTA: AR SECO
  } else if (umidade_ar > UMIDADE_AR_MAX * 100) {
    status_sistema = 6; // ALERTA: AR MUITO UMIDO
  }
  
  dados_sensor.status_sistema = status_sistema;
  
  // OTIMIZAÇÃO: Contador de erros
  if (status_sistema > 0) {
    contador_erros++;
  }
}
...
void atualizarDisplay() {
  ...
  switch (status_sistema) {
    ...
    case 5: lcd.print(F("ALERTA: AR SECO")); break;
    case 6: lcd.print(F("ALERTA: AR UMIDO")); break;
  }
  
  // OTIMIZAÇÃO: Indicadores visuais
  if (irrigacao_ativa) {
    lcd.setCursor(19, 2);
    lcd.print('*');
  }
...
// =============================================================================
// FUNÇÃO OTIMIZADA DE VERIFICAÇÃO DE CONEXÃO WiFi
// =============================================================================
...
void enviarDadosServidor() {
  if (!ENABLE_WIFI || !wifi_conectado) return;
  ...
  doc[F("contador_leituras")] = contador_leituras;
  doc[F("contador_erros")] = contador_erros;
...
// =============================================================================
// FUNÇÃO OTIMIZADA DE GERENCIAMENTO DO LED DE STATUS
// =============================================================================

void gerenciarLEDStatus() {
  static uint32_t ultimo_toggle = 0;
  static uint8_t led_estado = 0;
  
  uint32_t tempo_atual = millis();
  
  // OTIMIZAÇÃO: Padrões de LED baseados no status
  uint16_t intervalo_led;
  
  switch (status_sistema) {
    case 0: // OK
      intervalo_led = 2000; // Pisca a cada 2s
      break;
    case 1:
    case 2: // Alertas de temperatura
      intervalo_led = 500;  // Pisca rápido
      break;
    case 3:
    case 4: // Alertas de solo
      intervalo_led = 1000; // Pisca médio
      break;
    default: // Outros alertas
      intervalo_led = 750;  // Pisca médio-rápido
      break;
  }
  
  if (tempo_atual - ultimo_toggle >= intervalo_led) {
    led_estado = !led_estado;
    digitalWrite(LED_STATUS_PIN, led_estado);
...
void processarComandoSerial() {
  ...
  if (comando.startsWith(F("SETPOINT:"))) {
    ...
    Serial.printf(F("WiFi: %s\n"), wifi_conectado ? F("CONECTADO") : F("DESCONECTADO"));
    Serial.printf(F("Leituras: %d\n"), contador_leituras);
    Serial.printf(F("Erros: %d\n"), contador_erros);
    ...
    Serial.println(F("MEMORY - Exibe uso de memória"));
    Serial.println(F("DISPLAY:X - Muda modo display (0=normal, 1=detalhado)"));
  } else if (comando == F("RESET")) {
    integral = 0;
    erro_anterior = 0;
    contador_leituras = 0;
    contador_erros = 0;
    Serial.println(F("Contadores resetados"));
  } else if (comando == F("MEMORY")) {
    Serial.println(F("=== USO DE MEMÓRIA ==="));
  ...
  } else if (comando.startsWith(F("DISPLAY:"))) {
    uint8_t novo_modo = comando.substring(8).toInt();
    if (novo_modo <= 2) {
      modo_display = novo_modo;
      Serial.printf(F("Modo display alterado para: %d\n"), modo_display);
    } else {
      Serial.println(F("Erro: Modo deve estar entre 0 e 2"));
    }
  }
}
 ...
 * 
 * 3. VARIÁVEIS GLOBAIS:
 *    - Reduzidas e otimizadas
 *    - Tipos menores economizam ~50 bytes
 * 
 * 4. STRINGS:
 ...
 * 
 * ECONOMIA TOTAL ESTIMADA: ~300-400 bytes de RAM
 * MELHORIA DE PERFORMANCE: ~20-25% mais rápido
 * 
 * FUNCIONALIDADES IMPLEMENTADAS:
 * - Gerenciamento completo de sensores
 * - Display LCD com múltiplos modos
